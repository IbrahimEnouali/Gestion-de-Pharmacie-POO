...
class pharmacie :                      
    '''
génère une pharmacie dont on souhaite gérer, elle prend comme paramètre
une liste de clients et une liste de médicaments de la pharmacie 
    '''
    ...
    def liremedicament(self):                         
        '''      
vérifie si le médicament se trouve dans la pharmacie , si c'est le cas ,
il retourne l'objet médicament une fois on saisit un nom valide.
        '''
        ...
        while True :
            for med in self.listemedicaments :
                if med.nom == medicament :
                    return med
            print("Le médicament est invalide")
            print("1 : enregistrer le médicament")
            print("2 : Faute de frappe")
            a = int(input("Dans quel cas vous êtes: "))
            if a == 1 :
                self.enregistrement
            print("faites attention aux fautes de frappe")
            medicament = input('Veuillez réctifier le nom du médicament en un nom valide: ')
    @property
    def lireclient(self):                            # même travail realisé par liremedicament sauf cette fois on le fait pour les clients 
        ...
        while True :
            for clt in self.listeclients :
                if clt.nom == client :
                    return clt
            print("Le client est invalide")
            print("1 : enregistrer client")
            print("2 : Faute de frappe")
            a = int(input("Dans quel cas vous êtes: "))
            if a == 1 :
                self.enregistrement_client
            print("faites attention aux fautes de frappe")
            client = input('Veuillez réctifier le nom du client en un nom valide: ')
    @property
    def enregistrement(self):                        # enregistre un médicament 
        ...
        print('******le médicament est ajouté******')
    @property
    def enregistrement_client(self):                 #enregistre un client 
        ...
        print("******le client est ajouté******")
        
    def lirequantite(self,medicament):              
        '''
Méthode qu'on a ajouté, il vérifie si la quantité demandé est disponible dans le stock.
Elle retourne une quantité(int)une fois on saisit une quantité valide
        ''' 
